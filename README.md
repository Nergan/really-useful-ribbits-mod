...
- **Fisher.** Binds one nearby water block and one container. Sits on the bank with the fishing animation, runs vanilla-style fishing (splashes toward the bobber, then a catch), and stores loot in a single slot of 4. A full bag is emptied into the container with the matching open animation and sound. Right-click outlines the ribbit, the water tile and the container. Missing a bind plays the hurt croak and angry particles.
